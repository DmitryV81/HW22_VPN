Домашняя работа №22. Мосты, туннели и VPN.
...
Подводя итог тестирования можно сказать что TUN немного быстрее TAP. Так как устройства TUN ограничены протоколом IP, есть некоторые протоколы, для которых надо использовать только устройства TAP(к примеру DHCP, PXE).
